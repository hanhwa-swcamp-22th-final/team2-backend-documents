...
견적(PI) → 발주(PO) → 상업송장(CI) + 포장명세서(PL) + 출하지시서 + 생산지시서의 문서 흐름,
...
| production_orders | 생산지시서 | PO 기준 선택 생성 |
...
PI를 기반으로 생성되는 발주 문서입니다. PO 확정 시 하위 문서가 자동 생성되며, 생산지시서는 별도 선택 생성할 수 있습니다. PK는 문서번호 형식(VARCHAR(30))입니다.
...
PO 기준으로 선택 생성되는 생산 지시서입니다. PK는 문서번호 형식(VARCHAR(30))입니다.
